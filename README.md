...
| [scene_groups](#scene-groups-element) | yes      | List of scene group elements. |
...
### Scene groups element

Each element of the `scene_groups` list creates a group of related scenes.  Typically all of the scenes within a scene group operate on the same entities.

The `name` must be unique among all scene groups.

| Attribute         | Optional | Description |
| ----------------- | -------- |------------ |
| name              | no       | The name of the scene group to create. |
| [scenes](#scenes-element) | no       | The list of scenes to create within this scene group. |
| [scene_select](#scene-select-element) | yes      | When specified, creates a select entity whose options are the names of the scenes in the scene group. |

```yaml
  # Declare some scene groups
  scene_groups:
...
The `entities` map specifies the state to apply to an entity and it uses the same syntax as the built-in [scene integration](https://www.home-assistant.io/integrations/scene/).  The key of each entry is an entity ID and its value is the state.  For a simple state, provide a state string, such as "on" or "off".  For a complex state, provide a map of the state and its attributes as shown in the Developer Tools.
...
The `scene_select` element within a [`scene_groups` element](#scene-groups-element) creates a [select entity](https://www.home-assistant.io/integrations/select/) to control and observe the active scene among the scene group.

The options of the scene select entity are the names of the scenes in the scene group.  Selecting an option activates the corresponding scene.

A scene select entity is similar to the built-in Home Assistant [scene entity](https://www.home-assistant.io/docs/scene/).  They both can restore the state of a set of entities and they both use a similar method to define the desired state.  However they differ in two important ways:
...
The select entity's default name is the name of the scene group.  If you specify a `unique_id` then you can change the entity's ID and name in the Home Assistant UI.
...
Use the [`scene.turn_on` action](https://www.home-assistant.io/docs/scene/) to activate a scene entity.
